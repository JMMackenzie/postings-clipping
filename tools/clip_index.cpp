/* 
 * ../../script/clip_index /path/to/canonical/deepimpact /path/to/output/canonical/deepimpact-new splits.txt
...
int main(int argc, char **argv) {
    ...
    while(!docs.eof() && !freqs.eof()) {
        ...
        while (seq_count < d_seq_len) {
            ...
            // XXX Here we need to take a residual 
            // Decide which list...
            if (fdt <= split_bound) {
                low_docs.push_back(docid);
                low_freqs.push_back(fdt);
            } else{
                // We know the list is >= split_bound
                uint32_t low_part = split_bound; 
                uint32_t high_part = fdt - low_part;

                high_docs.push_back(docid);
                high_freqs.push_back(high_part);

                low_docs.push_back(docid);
                low_freqs.push_back(low_part);
            }
            
            ++seq_count;
        }

        // Guard for the case where the low list is empty...
        if (low_docs.size() == 0) {
            for (size_t i = 0; i < high_freqs.size(); ++i) {
                high_freqs[i] = high_freqs[i] + (split_bound + 1);
            }
        }
